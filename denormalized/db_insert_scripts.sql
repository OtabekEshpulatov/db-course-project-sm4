...
$$
    ...
    BEGIN
        ...
        for den_data in select distinct on (t.track_id) t.*,
                                                        alb.album_id,
                                                        alb.album_name,
                                                        art.artist_id,
                                                        art.artist_name,
                        ...
                        from public.tracks t
                                 JOIN public.album_tracks at on at.track_id = t.track_id
            ...
            loop
                ...
                if not exists(select * from denormalized_model.albums e where e.album_id = den_data.album_id) then
                    insert into denormalized_model.albums(album_id, album_name)
                    values (den_data.album_id, den_data.album_name);
                end if;

                -- inserting into denormalized_model.artists table
                if not exists(select * from denormalized_model.artists e where e.artist_id = den_data.artist_id) then
                ...
                if not exists(select *
                              from denormalized_model.playlists e
                              where e.playlist_id = den_data.playlist_id) then
                    insert into denormalized_model.playlists(playlist_id, playlist_url, year_)
                    values (den_data.playlist_id, den_data.playlist_id, den_data.year_);
                end if;

                -- inserting into denormalized_model.genres table
                if not exists(select * from denormalized_model.genres e where e.genre_id = den_data.genre_id) then
